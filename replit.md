...
- **In-App Google Play Update Notification (Task #1):** `GET /api/app/version` returns `{ minVersion, playStoreUrl }` from `site_settings` keys `min_android_version` and `play_store_url`. `APP_VERSION` constant in `client/src/lib/appVersion.ts`. After splash dismisses, App.tsx checks the version on Android native only and shows a non-dismissible `UpdateModal` if the installed version is outdated. Admin can edit both settings in the Settings tab.
- **Bootstrap Performance (Task #2):** AuthContext uses localStorage as an optimistic initial value and makes a single session check (previously made two sequential fetches). All pages except Home, Login, Register, Onboarding, and PhoneVerification are lazy-loaded via React.lazy + Suspense (Admin and Driver get dedicated chunks). `GET /api/home-data` combined endpoint returns categories + brands + cities + 12 featured products in one round-trip. SplashScreen calls `/api/home-data` immediately and seeds the React Query cache via `setQueryData`, so the Home page renders instantly after the splash with zero additional fetches.
