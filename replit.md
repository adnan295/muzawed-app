...
- **Bootstrap Performance (Task #2):** AuthContext now uses localStorage as an optimistic initial value and makes a single session check (previously made two sequential fetches). Admin and Driver pages are lazy-loaded via React.lazy + Suspense to reduce the initial JS bundle size. SplashScreen prefetches categories, brands, cities, and products into the React Query cache while the animation plays, so the Home page renders instantly after the splash.
